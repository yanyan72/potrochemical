...
仅关闭EWMA可见记录变化重置：跨切换条件Recall15.90%→34.17%，但延迟记录更新后clean FPR0.72%→41.18%。保留reset工程默认，carry实验开关；下一轮收尾评分组件批量接口与端到端使用，不扩展无边界算子搜索。结果/方法/状态/证据/交接已更新，外部论文未编辑。当前本地完成，上传待凭据与远端核对。
