...
## 2026-09-22：E1g重置单因素消融与项目验收边界

用户明确项目与论文分开，新增PROJECT_ACCEPTANCE。实现88a7c1d后完成logistics_e1g_20260922_v1，5种子×12场景×8组合；72项测试通过（17.60秒），旧评分重放240次、固定状态不变140次，源209/177及输出79hash通过。虚拟环境入口失效后按原依赖重建，无新依赖。test/质量/真实数据未评价。

仅关闭EWMA可见记录变化重置：跨切换条件Recall15.90%→34.17%，但延迟记录更新后clean FPR0.72%→41.18%。保留reset工程默认，carry实验开关；下一轮收尾评分组件批量接口与端到端使用，不扩展无边界算子搜索。结果/方法/状态/证据/交接已更新，外部论文未编辑。2026-09-22用户完成设备授权后，实现88a7c1d和结果c63b408已推送，远端核对为c63b408a2f85f5edc268eaad25f0737cb59e724c。本次只完成发布，不重复实验。

## 2026-09-21：E1f受控事件与CUSUM

实现1376b49后完成5种子×12场景×6组合正式实验logistics_e1f_20260921_v1，65项测试通过（20.38秒），编译/diff检查通过。审核源177/67和输出209哈希。新验证随机流、时长/数量/幅度单因素对照、随机与切换定点起点，以及经典CUSUM均已实现；未生成/评价test或质量。后处理一度使用不存在的clean_fpr列名，改为clean_channel_fpr后完成分析，未修改正式实验产物。

固定CUSUM的24组F1均低于EWMA且拖尾均增加，不升级主方法。切换前6步开始的条件EWMA Recall15.90%，切换后6步38.47%；相位效应包含重置及状态暴露，下一轮单独检验可见工况切换处理。方法、失败结果、论文对应、数据规范与交接均已更新。真实/质量任务仍待字段；外部论文正文未修改。2026-09-21用户完成设备授权后，实现1376b49和结果30f0b34已推送；远端核对为30f0b348d23773a6e571d8a3b1a78431fff37124。本次只完成发布，不重复实验。

## 2026-09-20：E1e完整有限窗口的负面结果

预设W=9，复用E1c输入与E1d逐点/EWMA存档评分。新增SMA、共同支持/全时间轴指标和事件覆盖，8项新测试、55项回归通过（11.53秒），compileall/diff通过。实现提交2b68a4f后完成logistics_e1e_20260920_v1，5种子×10场景×2残差×3时间方式；源177、基线67、本轮69个产物hash通过，test未评价。

SMA未达到减少拖尾且保留检出的目标：20组全时间轴F1/Recall均低于EWMA；共同支持19组F1较低、19组拖尾FPR较高。切换覆盖降为86.67%，完整记录启动期代价，不把不可用记为可靠。补充E1c比例预算同时改变事件时长的解释边界。决定不扩展窗口搜索，下一轮独立控制时长/频率并补更强基线；质量目标未定义，无预测或真实数据结论。本轮实现2b68a4f和结果a36004c在用户设备授权后已上传，git ls-remote核对远程main为a36004c06f7f787d4bdd602fe02472392ea91a39。

## 2026-09-20：E1d因果EWMA单模块消融

交付记录：用户完成设备授权后，实现7ca922a和结果ec519b5已推送至GitHub main；git ls-remote核对远程ec519b53065170d1b9b99ba753c312c42e792216。首次无凭据推送失败已解决；本次仅更新交付状态，不改动实验代码或产物。
