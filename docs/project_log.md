...
固定CUSUM的24组F1均低于EWMA且拖尾均增加，不升级主方法。切换前6步开始的条件EWMA Recall15.90%，切换后6步38.47%；相位效应包含重置及状态暴露，下一轮单独检验可见工况切换处理。方法、失败结果、论文对应、数据规范与交接均已更新。真实/质量任务仍待字段；外部论文正文未修改。当前本地完成，上传待凭据和远端核对。
