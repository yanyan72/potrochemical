...
SMA未达到减少拖尾且保留检出的目标：20组全时间轴F1/Recall均低于EWMA；共同支持19组F1较低、19组拖尾FPR较高。切换覆盖降为86.67%，完整记录启动期代价，不把不可用记为可靠。补充E1c比例预算同时改变事件时长的解释边界。决定不扩展窗口搜索，下一轮独立控制时长/频率并补更强基线；质量目标未定义，无预测或真实数据结论。本轮成果待提交与设备授权上传。
