...
- [x] 正式五种子×12场景完成；240次旧评分重放、140次稳态不变，79输出hash通过，图与结果已审查。
- [x] 项目状态、方法/结果、接口路径、证据矩阵和独立项目验收文档完成。
- [x] 结果提交c63b408与实现88a7c1d已推送，2026-09-22核对远端c63b408a2f85f5edc268eaad25f0737cb59e724c；随后提交交付状态记录。
...
跨切换条件Recall15.90%→34.17%，但延迟记录更新后clean FPR0.72%→41.18%；保留reset工程默认，carry只作实验选项。下一轮收尾批量评分组件与输入/模型契约，不继续无边界累积算子搜索。无质量/真实数据结果，test保留；项目验收与论文分开。
