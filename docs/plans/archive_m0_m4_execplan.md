# 历史 Milestone 0–4 执行计划（2026-09-18 归档，原过程记录保留）
...
- [x] 2026-09-10：完成 M4.3；正式 run、独立复现、图表审查和文档更新完成。验证集 q99 FPR 为 0.8547，门控结论为先修订 E1、暂不进入预测。
...
- 第一次 `/tmp` 复现审计用文本替换输出目录时误改了输入目录，导致 `FileNotFoundError`；修正临时配置后审计成功。正式配置和正式结果未受影响，六个核心产物与正式 run 逐字节一致。

## Outcomes and retrospective

Milestone 4 第一版 E1 已完成。正式 run `e1trust_20260910T035417735017Z_29f039af` 只从800个 normal-train clean 参考平方距离得到 q90=`7.497480759602013`、q99=`13.833465760805584`，并为4,800个时间点保存距离、可信度和分组。全套24项测试与语法编译通过，独立重跑的逐点CSV、校准JSON、三张指标表和PNG逐字节一致。

validation q99 Recall=`0.9302`，但 Precision=`0.0967`、F1=`0.1752`、FPR=`0.8547`；normal工况未污染点FPR仅`0.0078`，而compound、high-temperature和high-vibration均为`1.00`。PR-AUC=`0.4797`高于污染率`0.0896`，表明距离包含排序信号，但单一normal参考严重混淆合法工况变化与传感器污染。E1因此未通过进入预测实验的门控。下一步唯一任务是建立工况条件化训练参考并用完全相同的无泄漏评价重跑E1；test继续保留，预测模型继续暂停。
